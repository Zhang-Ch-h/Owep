...
import com.kclm.owep.convert.AdvisorConvert;
...
public class AdvisorServiceImpl implements AdvisorService {
    ...
    @Autowired
    private AdvisorConvert advisorConvert;
    ...
    @Override
    public List<AdvisorDTO> selectAdvisor() {
        try{
            List<User> users = userMapper.selectByType(4);
            List<AdvisorDTO> advisorDTOS = advisorConvert.toAdvisorDTO(users);
            return advisorDTOS;
        }catch (Exception e){
    ...
    @Override
    public List<AdvisorDTO> selectAdvisorByuserNameAndrealName(String userName, String realName) {
        try {
            List<User> user = userMapper.selectUserByuserNameAndrealName(userName, realName, 4); //4:咨询师
            List<AdvisorDTO> advisorDTOS = advisorConvert.toAdvisorDTO(user);
            return advisorDTOS;
        } catch (Exception e) {
            log.error(e.getMessage(), e);

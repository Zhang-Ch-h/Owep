...
public class AdvisorController {
    ...
    public ModelAndView advisorManagement() {
        return new ModelAndView("/user/advisorList");
    }

    @GetMapping("/advisorList/getTable")
    public List<AdvisorDTO> getAdvisorTable() {
        return advisorService.selectAdvisor();
    }

    @GetMapping("/advisorList/search")
    public Object searchAdvisor(@RequestParam(name = "userName", required = false) String userName,
                               @RequestParam(name = "realName", required = false) String realName) {
        try {
            List<AdvisorDTO> advisorDTOS = advisorService.selectAdvisorByuserNameAndrealName(userName, realName);
            Result result = new Result("200", "查询成功", advisorDTOS);
    ...
    public String deleteAdvisor(@RequestParam("id") Integer id) {
        try {
    ...
    public NodeDTO[] getGroupTreeView(@RequestParam("id") int userId) {//只是个字符串，没有相对应的数据库
        ...
        NodeDTO nodeDTO = advisorService.selectAllGroups();//获得完整树表
        List<NodeDTO> nodeList = nodeDTO.getNodes();
        List<Integer> groupIds = advisorService.selectGroupsByUserId(userId).getGroupIds();//获得已激活选择支id
        if (groupIds != null) {
    ...
    @PostMapping(value = "/advisorList/treeCheck_edit", produces = "application/json;charset=UTF-8")
    public String postGroupTreeViewEditAction(@RequestBody UserGroupDTO userGroupDTO) {
        ...
        advisorService.attachGroupToUser(userGroupMap);
        int feedback = advisorService.attachGroupToUser(userGroupMap);
        if (feedback == -2) {
            return "同步失败，意料之外的错误";
        }
        return "success";
    ...
    public ModelAndView addAdvisor(@RequestBody User user) {
        advisorService.save(user);
        return new ModelAndView("redirect:/user/advisorList");
    }

    @PostMapping(value = "/advisorList/checkUserName", produces = "text/html;charset=UTF-8")
    public String checkUserName(HttpServletRequest req) {
        //1. 获取请求参数
        String userName = req.getParameter("userName");
        System.out.println("用户请求的用户名: " + userName);
        //2. 判断
        if (advisorService.isUserNameUnique(userName)) {
            return "true";
        } else {
            return "false";
        }
    ...
    @PostMapping(value = "/advisorList/edit", consumes = "application/json;charset=UTF-8")
    public ModelAndView editAdvisor(@RequestBody User user) {
        log.debug("userPwd: " + user.getUserPwd());
        advisorService.update(user);
        return new ModelAndView("redirect:/user/advisorList");
    }

    @GetMapping("/advisorList/switch")
    public String switchStatus(@RequestParam("userId") int id, @RequestParam("status") int status) {
        advisorService.switchStatus(id, status);
        return "success";
    }

    @PostMapping("/advisorList/deleteByGroup")
    public String deleteAdvisorByGroup(@RequestBody List<Serializable> ids) {
        try {
            advisorService.deleteSelect(ids);
            return "success";
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return "failure";
    ...
    public ResponseEntity<String> export(HttpServletResponse response) {
        try {
            ...
            // 获取所有用户信息
            log.debug("开始导出用户信息");
            List<User> users = userRepository.findByUserTypeAndIsDelete(4, 1);

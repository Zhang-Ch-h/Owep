...
public class DefaultFtpProcessor implements FtpProcessor {
    ...
    private static final Logger log = LoggerFactory.getLogger(DefaultFtpProcessor.class);
    //连接池初始化标志
    ...
    private FTPClient getFtpClient() {
        ...
        try {
            for (int i = 0; i < ftpProperties.getRetryCount(); i++) {
                ftpClient = new FtpClientPooledObjectFactory(new FtpProperties()).makeObject().getObject();
